...
fn reads_common_fields_for_all_formats() {
    for name in FILES {
        ...
        assert!(
            t.length_ms >= 900 && t.length_ms <= 1200,
            "{name} length {}",
            t.length_ms
        );
    }
...
fn reads_musicbrainz_ids_for_all_formats() {
    for name in FILES {
        ...
        assert_eq!(
            t.recording_mbid.as_deref(),
            Some("aaaaaaaa-0000-0000-0000-000000000001"),
            "{name} recording"
        );
        assert_eq!(
            t.release_mbid.as_deref(),
            Some("bbbbbbbb-0000-0000-0000-000000000001"),
            "{name} release"
        );
        assert_eq!(
            t.release_group_mbid.as_deref(),
            Some("cccccccc-0000-0000-0000-000000000001"),
            "{name} rg"
        );
        assert_eq!(
            t.artist_mbid.as_deref(),
            Some("dddddddd-0000-0000-0000-000000000001"),
            "{name} artist"
        );
        assert_eq!(
            t.album_artist_mbid.as_deref(),
            Some("dddddddd-0000-0000-0000-000000000001"),
            "{name} albumartist"
        );
        assert_eq!(
            t.release_track_mbid.as_deref(),
            Some("eeeeeeee-0000-0000-0000-000000000001"),
            "{name} releasetrack"
        );
    }
}
...
fn reads_original_and_reissue_dates() {
    for name in FILES {
        ...
        assert!(
            t.reissue_date.as_deref().unwrap_or("").starts_with("2008"),
            "{name} reissue_date = {:?}",
            t.reissue_date
        );
        if MP4_FILES.contains(name) {
            ...
            assert_eq!(
                t.original_date, None,
                "{name} original_date should be None for MP4"
            );
        } else {
            assert!(
                t.original_date.as_deref().unwrap_or("").starts_with("1999"),
                "{name} original_date = {:?}",
                t.original_date
            );
        }
