...
Audio playback uses libmpv, which is **not** bundled. Install it from your distribution:
...
`org.urth.olivier.desktop` and the icons under `icons/hicolor/` can be copied into
`~/.local/share/applications` and `~/.local/share/icons/hicolor` respectively. The `.deb`/`.rpm`
